...
local function autoCollectLoop()
    while autoCollectEnabled do
            ...
            if part:IsA("BasePart") and part:FindFirstChild("Value") then
                ...
                if dist < radius then
                    ...
                    if collectEvent then
                        ...
                        game:GetService("ReplicatedStorage").RemoteEvent:FireServer(part)
                    end
                    found = true
                    break
                end
        ...
        if not found then task.wait(2) else task.wait(0.3) end
    end
end
...
local function farmLoop()
    ...
    while autoFarmEnabled do
        ...
        if farmMode == "TP" then workspace.Gravity = gravityNormal end
        task.wait(1)
    end
end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.F7 then
        ...
        if buildEnabled then autoBuild() end
    end
end)
...
local function updatePointList()
    ...
    if #names == 0 then pointList.Text = "Saved points: none"
    else pointList.Text = "Saved points: " .. table.concat(names, ", ") end
end
...
for i, name in ipairs(tabs) do
    tabButtons[name].MouseButton1Click:Connect(function()
        ...
        for _, btn in pairs(tabButtons) do btn.BackgroundColor3 = Color3.fromRGB(50,50,80) end
